! slightly modified from https://github.com/jacobwilliams/PowellOpt/blob/master/src/uobyqa.f90
!*****************************************************************************************
!>
...
!  by a trust region method that forms quadratic models by interpolation.
    ...
    subroutine uobyqa(n, x, rhobeg, rhoend, iprint, maxfun, calfun, &
        ...
        real(wp),intent(inout):: x(*)            !! initial values of the variables must be set in X(1),X(2),...,X(N). They
        ...
        real(kind=wp), intent(out), optional :: fval !! function value at minimum
        integer, intent(out), optional :: nfun !! # of function evaluations
        real(wp), allocatable :: w(:)
        integer :: npt,ixb,ixo,ixn,ixp,ipq,ipl,ih,ig,id,ivl,iw
        if (iprint > 3) then
           print*,"entered uobyqa, rhobeg, rhoend, iprint, maxfun, n ="
           print*,rhobeg, rhoend, iprint, maxfun, n
           print "('x:',*(1x,f12.6))", x(1:n)
        end if
    ...
    subroutine uobyqb(n, x, rhobeg, rhoend, iprint, maxfun, npt, xbase, xopt, xnew, xpt, &
                      pq, pl, h, g, d, vlag, w, calfun, fval, nfun)

        implicit real (wp) (a-h, o-z)

        dimension x (*), xbase (*), xopt (*), xnew (*), xpt (npt,*), pq (*), pl (npt,*), &
                  h (n,*), g (*), d (*), vlag (*), w (*)
        real(wp), intent(out), optional :: fval
        integer, intent(out), optional :: nfun
