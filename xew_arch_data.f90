program xew_arch_data
! read asset prices, compute returns and fit GARCH and equal-weight ARCH models to them
...
use           garch_mod, only: fit_garch, nparam_garch, nparam_gjr_garch, &
                               ...
                               nparam_ew_arch, fit_ew_arch, ew_arch_str
use         obj_fun_mod, only: garch_model, narch
...
character (len=*), parameter :: infile = "ew_arch_data.dat"
...
real(kind=dp) :: ew_arch_par(nparam_ew_arch), garch_par(nparam_garch), gjr_garch_par(nparam_gjr_garch)  ! estimated parameters
...
read (iu,*) narch ! # of lags of equal-weight ARCH model
...
do idist=1,ndist
   ...
   if (any(garch_models == ew_arch_str)) then
      call fit_ew_arch(ret, dist(idist), ew_arch_par, logL, info, niter=niter_nm, &
         sigma=sigma_est, max_iter=max_iter_nm, tol=lik_tol, alpha0=alpha0)
      nfits = nfits + 1
      call print_garch_results(ew_arch_str, dist(idist), ew_arch_par, &
...
print "(/,a)", "(2) finished xew_arch_data"
end program xew_arch_data
